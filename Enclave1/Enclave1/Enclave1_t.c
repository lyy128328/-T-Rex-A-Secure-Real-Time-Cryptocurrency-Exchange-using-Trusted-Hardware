...
static sgx_status_t SGX_CDECL sgx_foo(void* pms)
{
	...
	size_t _tmp_msg_len = ms->ms_msg_len;
	size_t _len_message = _tmp_msg_len;
	...
	foo(_in_message, _tmp_msg_len, _in_buf, _tmp_buf_len);
	if (_in_buf) {
...
static sgx_status_t SGX_CDECL sgx_sign(void* pms)
{
	...
	if (_tmp_buff != NULL && _len_buff != 0) {
		if ( _len_buff % sizeof(*_tmp_buff) != 0)
		{
			status = SGX_ERROR_INVALID_PARAMETER;
			goto err;
		}
		if ((_in_buff = (char*)malloc(_len_buff)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}
